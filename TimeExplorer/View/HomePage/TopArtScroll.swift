...
struct TopArtScroll: View {
    var cardSize:CGSize = .init(width: totalWidth * 0.65, height: totalHeight * 0.5)
...
//        return w
    ...
    func computeParams(idx:Int) -> (Bool,CGFloat,CGFloat,Double){
        ...
        let x_off:CGFloat = isViewing ? self.SP.dy_off : -cardSize.width * (isViewing && diff == 0 ? 0 : diff * 0.5)
