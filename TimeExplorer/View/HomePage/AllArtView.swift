...
struct ArtViewCard:View{
    ...
    var body: some View{
        ZStack(alignment: .center) {
            ImageView(url: self.cardData.img, width: self.cardSize.width, height: self.cardSize.height, contentMode: .fill, alignment: .center, testMode: false, autoHeight: false)
            lightbottomShadow
            VStack(alignment: .leading, spacing: 10) {
                Spacer()
